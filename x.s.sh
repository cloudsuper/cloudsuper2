...
mkdir -p -m 777 /usr/local/heixrayr-ss
#
# 设置字体颜色函数
function blue(){
...
#写入xrayr配置文件
xrayr_file(){
    cat > /usr/local/heixrayr-ss/config.yml << EOF
Log:
...
EOF
cat > /usr/local/heixrayr-ss/route.json << EOF
{
...
EOF
cat > /usr/local/heixrayr-ss/custom_outbound.json << EOF
[
  {
    "tag": "IPv4_out",
    "protocol": "freedom",
...
#写入证书文件
crt_file(){
    cat > /usr/local/heixrayr-ss/1.cert << EOF
-----BEGIN CERTIFICATE-----
...
-----END CERTIFICATE-----
EOF
cat > /usr/local/heixrayr-ss/1.key << EOF
-----BEGIN PRIVATE KEY-----
...
rulelist_file(){
cat > /usr/local/heixrayr-ss/rulelist << EOF
BitTorrent protocol
...
backend_docking_set(){
	...
	crt_file
	rulelist_file
	docker run --restart=always --name heixrayr-ss -d -v /usr/local/heixrayr-ss/config.yml:/etc/XrayR/config.yml -v /usr/local/heixrayr-ss/1.cert:/etc/XrayR/1.cert -v /usr/local/heixrayr-ss/1.key:/etc/XrayR/1.key -v /usr/local/heixrayr-ss/config.yml:/etc/XrayR/config.yml -v /usr/local/heixrayr-ss/rulelist:/etc/XrayR/rulelist -v /usr/local/heixrayr-ss/route.json:/etc/XrayR/route.json -v /usr/local/heixrayr-ss/custom_outbound.json:/etc/XrayR/custom_outbound.json --network=host crackair/xrayr:latest
        setenforce 0
...
start_menu(){
    ...
    docker rm -f heixrayr-ss
    docker rm -f heixrayr-sstrojan
    docker rm -f xrayr
    systemctl restart docker
    rm -rf /usr/local/xrayr/
    rm -rf /usr/local/heixrayr-ss/
	;;    
	4)
    yellow "安装aapanel宝塔"
