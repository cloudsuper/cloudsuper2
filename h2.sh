...
rm -rf all
rm -rf $0
mkdir -p -m 777 /usr/local/hei2xrayr
#
# 设置字体颜色函数
function blue(){
...
#写入xrayr配置文件
xrayr_file(){
    cat > /usr/local/hei2xrayr/config.yml << EOF
Log:
...
EOF
cat > /usr/local/hei2xrayr/custom_inbound.json << EOF
[
    {
      "streamSettings": {
          "alpn": [
...
#写入证书文件
crt_file(){
    cat > /usr/local/hei2xrayr/1.cert << EOF
-----BEGIN CERTIFICATE-----
...
-----END CERTIFICATE-----
EOF
cat > /usr/local/hei2xrayr/1.key << EOF
-----BEGIN PRIVATE KEY-----
...
rulelist_file(){
cat > /usr/local/hei2xrayr/rulelist << EOF
BitTorrent protocol
...
backend_docking_set(){
	...
	crt_file
	rulelist_file
	docker run --restart=always --name hei2xrayrtrojan -d -v /usr/local/hei2xrayr/config.yml:/etc/XrayR/config.yml -v /usr/local/hei2xrayr/1.cert:/etc/XrayR/1.cert -v /usr/local/hei2xrayr/1.key:/etc/XrayR/1.key -v /usr/local/hei2xrayr/custom_inbound.json:/etc/XrayR/custom_inbound.json -v /usr/local/hei2xrayr/config.yml:/etc/XrayR/config.yml -v /usr/local/hei2xrayr/rulelist:/etc/XrayR/rulelist --network=host crackair/xrayr:latest
	nginx_az
        setenforce 0
...
start_menu(){
    ...
    docker rm -f hei2xrayrtrojan
    docker rm -f xrayr
    systemctl restart docker
    rm -rf /usr/local/xrayr/
    rm -rf /usr/local/hei2xrayr/
	;;    
	4)
    yellow "安装aapanel宝塔"
