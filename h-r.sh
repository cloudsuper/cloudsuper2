...
backend_docking_set(){
	...
	crt_file
	rulelist_file
	docker run --restart=always --name heixrayr -d -v /usr/local/heixrayr/config.yml:/app/config.yml -v /usr/local/heixrayr/1.cert:/app/1.cert -v /usr/local/heixrayr/1.key:/app/1.key -v /usr/local/heixrayr/config.yml:/app/config.yml -v /usr/local/heixrayr/rulelist:/app/rulelist -v /usr/local/heixrayr/route.json:/app/route.json -v /usr/local/heixrayr/custom_outbound.json:/app/custom_outbound.json --network=host tanzichao6/heixrayr-reality:v1
        setenforce 0
